...
export default class App extends Component {
    ...
    search = (term, items) => {
        console.log('search ' + this.state.activeStatus);


        items = this.filterByTaskBlock(items, this.state.activeStatus);
        if (term.length === 0) {
            return items;
        } else {
    ...
    filterByTaskBlock = (elements, filter) => {
        if (filter === 'done'){
    ...
    onInfoBtnClick = (status) => {
        this.setState({activeStatus:status});
    };

    render() {

        const {term, tasks} = this.state;
        ...
        return (
            ...
            <div className="todo-app">
                ...
                <div className="top-panel d-flex">
                    ...
                    <ItemStatusFilter onInfoBtnClick={this.onInfoBtnClick}/>
                </div>
